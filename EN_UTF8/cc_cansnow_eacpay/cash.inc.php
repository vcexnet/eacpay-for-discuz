...
if ($action == 'cash') {
	...
	if(submitcheck('addsubmit')){
		...
		if(!$cash_address){			
			showmessage("Withdrawal address must be filled in");
		}
		...
		if ($amount < $moneymin || $amount > 99999 || $amount == '') {
			showmessage("The recharge amount cannot be less than".$moneymin." or greater than",'index.php');
		}
		$d = array();
		$d['extcredits'.$csetting['moneytype']] = 0-$vo['amount'];
		updatemembercount($vo['uid'],$d,false,'cash','0','','Chargeback','Chargeback');
		...
		showmessage('Application successful, waiting for review',"index.php");
	}
}elseif($action == 'log'){
