...
j(function() {
    ...
    j('.long-logo.mbw ul').addClass('flex_row').append('<li class="z">' +
        ...
        '<span class="xs2">EACPAY区块链充值</span></label>' +
        '</li>');
    var htm = '<tr class="cc_cansnow_eacpay"><th>EAC即时价</th><td colspan="2"><div id="exchangeData">' + exchangeData + '</div></tr>';
    ...
    j('#addfundsform').after(['<div class="cc_cansnow_eacpay"><div class="flex_row eacpay_remark">',
        '<div class="flex_1">' +
        '   <p>人民币现金 1元 = ' + moneybl + ' ' + moneytype_title + '</p>',
        '   <p>单次最低充值 ' + moneymin + ',单次最高充值 ,最近30天最高充值 10000RMB</p>',
    ...
    j('#addfundssubmit_btn').on('click', function() {
        if ($('apitype_eacpay').checked) {
            j.getJSON('plugin.php?id=cc_cansnow_eacpay:recharge&action=geteac&submit=1&amount=' + j('#exchangeamount').val(), function(d) {
                ...
                j('#qrcode').html('<div><img src="' + d.qrcode + '" width="150"/><div>EACPAY钱包扫码支付</div></div>');
                timeId = setTimeout(check, 3000); //开始任务
            });
            return false;
        }
    });
